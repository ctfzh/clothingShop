...
public class HomePresenter extends BasePresenter<HomeFragment>
{
    ...
    public void loadHome(String key, final boolean reloadData)
    {
        ...
        {
            view.response(view.getActivity(), R.string.no_more_data);
            view.stopRefresh();
            return;
        }
        ...
        CCHttpHelper
                ...
                .subscribe(new Subscriber<ResponseTemplate>()
                {
                    ...
                    public void onError(Throwable e)
                    {
                        if (view == null)
                            return;
                        view.response(view.getActivity(), view.getString(R.string.network_error) + TextEmotion.ganga);
                        view.stopRefresh();
                    }
                    ...
                    public void onNext(ResponseTemplate responseTemplate)
                    {
                        ...
                        {
                            view.stopRefresh();
                            view.response(view.getActivity(), view.getString(R.string.no_more_data) + TextEmotion.ganga);
                            return;
                        }
